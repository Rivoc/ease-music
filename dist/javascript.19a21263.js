...
//这个js的目的是创建一个div标签，这将buttons.svg的代码放进这个标签里，从而避免将buttons.svg的代码全部拷贝到 index.html里使得冗长且不好维护
...
function () {
  ...
  _createClass(Player, [{
    ...
    value: function start() {
      ...
      fetch("https://rivoc.github.io/data-mock/voo-music/music-list.json").then(function (res) {
        return res.json();
      }).then(function (data) {
        console.log(data);
    ...
    value: function loadSong() {
      ...
      console.log(this.audio.src); //1️⃣将页面头部的“歌曲名”和“作者-专辑名”直接渲染出来：
    ...
    value: function setLyrics(lyrics) {
      ...
      lyricsArr.filter(function (line) {
        ...
        //6️⃣：排序好后，对里边的每一条运用以下逻辑：
        var node = document.createElement("p"); //6️⃣-①：在页面创建一个 p 标签；
    ...
    value: function locateLyric() {
      ...
      var currentTime = this.audio.currentTime * 1000; //1️⃣获取到当前音乐的时间毫秒数；
      ...
      if (currentTime > nextLineTime && this.lyricIndex < this.lyricsArr.length - 1) {
        this.lyricIndex++;
        var node = this.$('[data-time="' + this.lyricsArr[this.lyricIndex][0] + '"]'); // 3️⃣调用 setLyricToCenter 方法，并传入参数 node ；

        if (node) this.setLyricToCenter(node); //4️⃣我们一并在这里把 panel-effect 面板上的两行歌词也渲染出来。
    ...
    value: function setLyricToCenter(node) {
      console.log(node); //偏移的高度；

      var translateY = node.offsetTop - this.$(".panel-lyrics").offsetHeight / 2; //1️⃣对计算出的“高度”做一个判断，看看是“情况 1”(到屏幕中间的歌词部分）还是“情况 2”；

      translateY = translateY > 0 ? translateY : 0; //2️⃣做出判断后，把歌词放到“歌词面板”的中央；

      this.$(".panel-lyrics .container").style.transform = "translateY(-".concat(translateY, "px)"); //3️⃣歌词“高亮”的设置：
    ...
    value: function bind() {
      ...
      this.audio.ontimeupdate = function () {
        self.locateLyric(); //同时还会设置“播放进度条”，我们一并在这里调用。
